...
public class CreateUserUseCase implements IUseCaseExecute<CreateUserRequest, UserResponse> {
    ...
    public Mono<UserResponse> execute(CreateUserRequest cmd) {
        return userRepository.findByDocumentId(cmd.getDocumentId())
                .flatMap(existingUser -> Mono.error(new ConflictException("User with the document id: " + cmd.getDocumentId()+ " already exists")))
                .then(Mono.defer(() -> {
